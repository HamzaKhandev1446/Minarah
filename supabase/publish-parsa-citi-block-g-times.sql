...
-- Publishes only Block G; does not verify the mosque or change Block A.
begin;
...
begin
  ...
  where slug = 'parsa-citi-block-g-masjid' and timezone = 'Asia/Karachi'
    and verification_status <> 'rejected' and not is_synthetic
  for update;
  if not found then
    raise exception 'Block G mosque is missing or unavailable; run its location import first';
  end if;
  ...
  values
    (schedule_id_value, 'fajr',    time '05:30'),
    (schedule_id_value, 'dhuhr',   time '13:15'),
    (schedule_id_value, 'asr',     time '17:15'),
    (schedule_id_value, 'maghrib', time '18:42'),
    (schedule_id_value, 'isha',    time '20:15');

  insert into public.jumuah_sessions (schedule_id, position, local_time, label)
  values (schedule_id_value, 1, time '13:45', 'Jumuah');
...
where m.slug = 'parsa-citi-block-g-masjid' and s.status = 'published'
